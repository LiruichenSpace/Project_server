...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1)):
    ...
    n_dim = tensor.dim()
    if n_dim == 4:#如果squeeze后还是4维的话，说明是一个batch，则将多张图片组织成一个grid
        n_img = len(tensor)
...
def read_seq_imgs(img_seq_path):
    ...
    #re是正则表达式模块，search模块是找到第一个匹配项，basename是获取文件名，group可以用于得到目标字符串中对应分组的子串
    #这里应该是返回文件名中的数字，并组织成数组
    img_l = [read_image(v) for v in img_path_l]
    # stack to TCHW, RGB, [0,1], torch
    #由于cv2的读取是H，W，C，并且通道轴是BGR，需要进行调整
    imgs = np.stack(img_l, axis=0)#将多个在0维上层叠，它会使目标维度增加
    imgs = imgs[:, :, :, [2, 1, 0]]#调整通道
