...
def parse(opt_path, is_train=True):
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = gpu_list #此处的os.environ相当于设置环境变量
    ...
    for key, path in opt['path'].items():
        if path and key in opt['path'] and key != 'strict_load':
            opt['path'][key] = osp.expanduser(path) #expanduser可以将path前面的~或者~user展开为用户的home路径
    opt['path']['root'] = osp.abspath(osp.join(__file__, osp.pardir, osp.pardir, osp.pardir))
    # 从当前位置回退三层，获得项目主目录路径
    if is_train:#如果是训练，设置保存的路径信息
        #输出文件夹
